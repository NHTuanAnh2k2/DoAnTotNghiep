...
public interface TrangChuRepository extends JpaRepository<SanPham, Integer> {
    ...
    @Query(nativeQuery = true, value = """
               ...
               GiamGia AS (
                   SELECT spct.id AS spctId, dg.giatrigiam AS giaGiam
               ...
               LEFT JOIN GiamGia giamgia ON spct.id = giamgia.spctId
    ...
    @Query(nativeQuery = true, value = """
              ...
              SELECT sp.id, sp.TenSanPham, sct.GiaTien, anhdd.tenanh, SUM(ct.SoLuong) AS TongSoLuong, kc.soLuongKichCo, ms.soLuongMauSac, COALESCE(giamgia.giaGiam, 0) AS giaGiam
              FROM HoaDonChiTiet ct
              JOIN SanPhamChiTiet sct ON ct.IdSanPhamChiTiet = sct.Id
              JOIN HoaDon hd ON ct.IdHoaDon = hd.Id
              ...
              WHERE MONTH(hd.lancapnhatcuoi) = MONTH(GETDATE())
                AND YEAR(hd.lancapnhatcuoi) = YEAR(GETDATE())
                AND hd.trangthai = 5
              GROUP BY sp.id, sp.TenSanPham, sct.GiaTien, anhdd.tenanh, kc.soLuongKichCo, ms.soLuongMauSac, giamgia.giaGiam
